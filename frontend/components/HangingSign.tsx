...
export default function HangingSign({ icon, title, delay, href, onClick }: HangingSignProps) {
  const content = (
    <div className="pixel-box bg-primary border-primary-dark p-1 transition-transform group-hover:-translate-y-1 group-active:translate-y-1">
      ...
      <div className="absolute top-0 right-0 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-accent-gold border border-black"></div>
      <div className="absolute bottom-0 left-0 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-accent-gold border border-black"></div>
      <div className="absolute bottom-0 right-0 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-accent-gold border border-black"></div>
    </div>
  );
